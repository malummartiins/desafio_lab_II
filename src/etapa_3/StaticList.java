...
public class StaticList<E> implements List<E> {
	...
	public int contaElementos(E el) throws IllegalArgumentException {
		if (el == null) throw new IllegalArgumentException("Elemento não pode ser null");
		return contaElementos(el, 0, 0);
	}
	
	private int contaElementos(E el, int index, int qtdElementos) {
		if (index == numElements) return qtdElementos;
		if (get(index).equals(el)) { // Use .equals() para comparar objetos
			qtdElementos++;
		}
		return contaElementos(el, index + 1, qtdElementos);
	}
